...
### Phase 8 — Dark mode ✅ *(done 2026-07-20, requested outside the original audit scope)*

- [x] **8.1** System-preference dark mode (`prefers-color-scheme`) + a small sun/moon toggle in the nav (desktop) and mobile drawer header, both instances kept in sync. Every themed token in `variables.css` carries a `light-dark()` pair; `color-scheme` cascade (system → explicit `html[data-theme]` override) is pure CSS and reacts live to OS changes with no JS. `public/theme-init.js` (external — the CSP is `script-src 'self'`) restores a saved override before first paint, no flash. Full write-up of the mechanism and every hurdle it flushed out (unthemed decorative SVGs, the data-URI select chevron, shadow tint direction, a resurfaced dangling-selector bug that silently broke the About timeline's grid, one real contrast gap in `--accent-warm`) is in commit `af206cd`. **Lighthouse: 100 a11y / 100 best-practices / 100 SEO on every page in both themes.**
- [x] **8.2** Housekeeping found along the way: an earlier `git add -A` had accidentally committed a stray Lighthouse Chrome profile (2,127 files) into the repo — removed from tracking, `.gitignore` guarded against recurrence (commit `e0235a6`). **Note: this bloat is still in git history (not rewritten — ask before squashing/filter-branching if repo size matters).**
